...
export default function PaymentPage() {
  ...
  const handlePay = async (e) => {
    ...
    if (parseFloat(amount) < 1) return setError('Minimum amount is ₹1');
    if (parseFloat(amount) > 500000) return setError('Maximum amount is ₹5,00,000');
  ...
  if (loading) return <div className="payment-wrapper">Loading...</div>;
